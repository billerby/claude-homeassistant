...
pull: check-env
	...
	@rsync -avz --delete --exclude-from=.rsync-excludes --rsync-path="sudo rsync" $(HA_HOST):$(HA_REMOTE_PATH) $(LOCAL_CONFIG_PATH)
...
push: check-env
	...
	@rsync -avz --delete --exclude-from=.rsync-excludes --rsync-path="sudo rsync" $(LOCAL_CONFIG_PATH) $(HA_HOST):$(HA_REMOTE_PATH)
